...
namespace BirthdayGreetings.Email.Test
{
    public class EmailGreetingsGatewayTest
    {
        ...
        public void Should_send_one_email_when_it_is_required_to_send_a_list_with_only_one_greeting()
        {
            ...
            A.CallTo(() => _emailChannel.Send(A<MailMessage>.That.Matches(mm => CheckMailMessage(greeting, mm)))).MustHaveHappened(Repeated.Exactly.Once);
        }

        private static bool CheckMailMessage(GreetingDto greeting, MailMessage mailMessage)
        {
